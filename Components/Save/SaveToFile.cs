...
namespace Dox.Components.SaveToFile
{
    public class SaveToFile
    {
        ...
        public static string IPDatabase = CurrentDir + "\\IPDatabase\\";
        ...
        public static void IP_MakeTextFile(IPAddress IP, string status, string type, string continent, string continent_code, string country_code, string phone, string region, string city, string latitude, string longitude, string isp, string currency)
        {
            try
            {
                ...
                if (!Directory.Exists(IPDatabase))
                {
                    Directory.CreateDirectory(IPDatabase);
                    Thread.Sleep(2000);
                }
                if (Directory.Exists(IPDatabase))
                {
                    Colorful.Console.Write("[Log] Writing data to " + IPDatabase + IP + ".txt", Color.LightGoldenrodYellow);
                    if (File.Exists(IPDatabase + IP + ".txt"))
                    {
                        ...
                        switch (OverwriteData)
                        {
                            case "Y":
                                try
                                {
                                    File.Delete(IPDatabase + IP + ".txt");
                                    Thread.Sleep(2000);
                                    Colorful.Console.WriteLine("[Log] Successfully deleted {0} from the database", Color.LightGoldenrodYellow, IPDatabase + IP + ".txt");
                                }
                    ...
                    Colorful.Console.Write("\n[Log] Completed writing data to " + IPDatabase + IP + ".txt\n", Color.LightGoldenrodYellow);
                    Colorful.Console.Write("[Input] Would you like to go back to main menu? (Y/N): ", Color.LightGoldenrodYellow);
