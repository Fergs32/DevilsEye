...
namespace Dox.Components.AccountLeaks
{
    internal class Scraper
    {
        ...
        public static void Start()
        {
            using (HttpRequest req = new HttpRequest())
            {
                ...
                Console.WriteLine($"[!] Downloaded All files to {Download.CurrentDir}");
        ...
        {
            Regex regex = new Regex("<td class=\"td-time\">(.*?)</td>", RegexOptions.IgnoreCase);
            Match m = regex.Match(resp);
            while (m.Success)
            {
                Group g = m.Groups[1];
    ...
    {
        public static string CurrentDir = Directory.GetCurrentDirectory() + "\\Misc\\AccountScraper\\";
